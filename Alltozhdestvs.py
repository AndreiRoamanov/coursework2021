...
#        print(endzn)
    ...
    while nextloop < 19:
        ...
        if nextloop % 2 == 1: # на данном этапе применяется только тождество 2.
            predlen = minlen - 1
            modes = 0
            wasfst = -1
            wasscnd = 0
...
# ВЫВОД ПОЛИНОМОВ, ВХОДЯЩИХ В ЧИСЛО МИНИМАЛЬНЫХ ДЛЯ ДАННОГО ШАГА
    ...
    print("ourminpol = ", allminfullpols)
...
#### получен обобщённый полином при помощи тождеств 1-2 (через полином Жегалкина). Это занимает немного времени.

    promtozhd_cnt = n - 3
    cnt_tozhd = promtozhd_cnt
    for mas_ind in range(0, n - 2): # не сччитаем первое и второе тождество как отдельные. Сначала проход по 3, потом по 3 и 4, потом 3,4,5 и т.д.
        cnt_tozhd = promtozhd_cnt
        pol = allminfullpols
        i = 0
        print("here promtozhd = ", promtozhd_cnt, "  and pol = ", pol)
        thismon = 0
        maxsymb = 0
        lenthismon = 0
        cntallminpols = 0
        ...
        while bufffpol > 0:
            ...
            while bufnewmon > 0:
                ...
                if litt != 0:
                    cntthismon +=1
                bufnewmon = bufnewmon // 10
            lenmon.append(cntthismon)
            bufffpol = bufffpol // 10 ** n
    ...
    # ТОЖДЕСТВА
        ...
        print("MON NOW = ", mon)
        print("MONLEN NOW = ", lenmon)
        print("NOW promtozhd_cnt = ", promtozhd_cnt, "cnt_tozhd = ", cnt_tozhd)
        ...
        while nextloop < 9: # 19?!
            if nextloop % 2 == 1:
                ...
                allherepols = []
                allherepols.append(polnow)
                t2cnt = 0
               # for t2cnt in range(0, len(newnumofpols)):
                polsget = 0
                ends = 0
                while t2cnt < len(newnumofpols):
                    if ends == 1:
                        break
          #          print("here with t2cnt = ", t2cnt, " and newnumofpols = ", newnumofpols)
                    
                    tozhd = 3
                    notnow='''if goodcnt == 1:
                        t2cnt = t2cnt + 1
                    if t2cnt == len(t2pols) or len(t2pols) > 2 ** (n - cnt_tozhd + 2):
                        break'''
                    ...
                    #print("newnumofpols = ", newnumofpols, " pol = ", pol)
                            ...
                            while (True):
                                ...
                                while u < predlen - 1:
                                    ...
                                    while z < predlen:
                                        ...
                                        if ravn == cnt_tozhd: # 1 - здесь 1 - тождество для n - 1 переменных
                                            prodterm1 = prodterm[u]
                                            prodterm2 = prodterm[z]
                                            newprodterms = []
                                            strnewprodterms = []
                                            for jjj in range(0, n - cnt_tozhd):
                                                newprodterms.append(-1)
                                                strnewprodterms.append('')
                                            nrvind = 0
                                            strprodterm1 = str(prodterm1)
                                            if strprodterm1 == '3':
                                                strprodterm1 = '0'
                                            ...
                                            for zzz in range (len(strprodterm1)):
                                                if strprodterm1[zzz] == strprodterm2[zzz]:
                                                    for jjj in range(0, n - cnt_tozhd):
                                                        strnewprodterms[jjj] = strnewprodterms[jjj] + strprodterm2[zzz]
                                                elif strprodterm1[zzz] != strprodterm2[zzz]:
                                                    if modess == 0:
                                                        for jjj in range(0, n - cnt_tozhd):
                                                            if nrvind == jjj:
                                                                if strprodterm1[zzz] == '1' and strprodterm2[zzz] == '2' or strprodterm1[zzz] == '2' and strprodterm2[zzz] == '1':
                                                                    strnewprodterms[jjj] = strnewprodterms[jjj] + '0'
                                                                elif strprodterm1[zzz] == '0' and strprodterm2[zzz] == '2' or strprodterm1[zzz] == '2' and strprodterm2[zzz] == '0':
                                                                    strnewprodterms[jjj] = strnewprodterms[jjj] + '1'
                                                                elif strprodterm1[zzz] == '1' and strprodterm2[zzz] == '0' or strprodterm1[zzz] == '0' and strprodterm2[zzz] == '1':
                                                                    strnewprodterms[jjj] = strnewprodterms[jjj] + '2'
                                                            elif nrvind < jjj:
                                                                strnewprodterms[jjj] = strnewprodterms[jjj] + strprodterm2[zzz]
                                                            else:
                                                                strnewprodterms[jjj] = strnewprodterms[jjj] + strprodterm1[zzz]
                                                        nrvind = nrvind + 1
                                                    elif modess == 1:
                                                        for jjj in range(0, n - cnt_tozhd):
                                                            if nrvind == jjj:
                                                        ...
                                                        nrvind = nrvind + 1
                                            for jjj in range(0, n - cnt_tozhd):
                                                newprodterms[jjj] = int(strnewprodterms[jjj])
                                            nowlen = nowlen + n - cnt_tozhd
                                            if newprodscnt < len(prodterm):
                                                for jjj in range(0, n - cnt_tozhd):
                                                    prodterm[newprodscnt + jjj] = newprodterms[jjj]
                                            else:
                                                for jjj in range(0, n - cnt_tozhd):
                                                    prodterm.append(newprodterms[jjj])
                                            newprodscnt = newprodscnt + n - cnt_tozhd
                                            prodterm[z] = 4 # те значения, которые были старыми, стираем, так как они соединены в новое # 4 - символ, что его вовсе нет
                                            prodterm[u] = 4
                                            thislen = 0
                                            thisesop = 0
                                            ...
                                            for kkk in range(0, len(monfromthisesop)):
                                                if monfromthisesop[kkk] == 0 and kkk == 0:
                                                    newmon = newmon * 10 ** n + 3
                                                else:
                                ...
                                if len(bigallprodterms) != prommlen:
                                    ...
                                    if wasfst == nowlen - 1:
                                        newnumofpolsbuf = []
                                        [newnumofpolsbuf.append(item) for item in t2pols if item not in newnumofpolsbuf]
                                        break
                                    
                    [allherepols.append(item) for item in newnumofpolsbuf if item not in allherepols]
                    if len(allherepols) > 2 ** (n + n - cnt_tozhd):
                        break
                    if t2cnt == len(newnumofpols) - 1:
                        newnumofpols = []
                        [newnumofpols.append(item) for item in newnumofpolsbuf if item not in newnumofpols]
                        t2cnt = -1
                        if polsget > 0:  #!!!! - увеличивает число полиномов, получаемых в результате тождеств
                            polsget = 0
                            cnt_tozhd = cnt_tozhd + 1
                            if cnt_tozhd >= n - 1:   # Промежуточные тесты показали, что хватает n - 2. Максимум, который можно сделать - n - 1.
                                ends = 1
                        polsget = polsget + 1
                    t2cnt = t2cnt + 1
            if nextloop == 1:
            ...
            while (True) and nextloop % 2 == 0:
    #            print("NOW PRODTERM = ", prodterm, " mon = ", mon)
                prodterm = []
                cntmon = len(mon)
                for formon in range (0, cntmon):
                    prodterm.append(mon[formon])
                ...
                # избавление от первоначальных отрицаний
                '''if firstly == 1:
    #com1804                #print("pred negs remove prodterm = ", prodterm)
                    otrnowlen = nowlen
                    while u < otrnowlen:
                        u = u + 1
                        prodterm0 = prodterm[u]
                        ...
                        while bufprodterm0 > 0:
                            ...
                            if litnow == 2:
                                ...
                                mon.append(prodterm[u] + 10 ** cntforneg)
                                lenmon.append(lenprodterm[u] + 1)
                                #ccc=prodterm[u] = prodterm[u] - litnow * 10 ** cntforneg
                                #lenprodterm[u] = lenprodterm[u] - 1 # так как уменьшается длина - мы вычёркиваем один литерал
                                #prodterm.append(prodterm[u] + 10 ** cntforneg)
                                #lenprodterm.append(lenprodterm[u] + 1)
                             #   prodterm[newprodscnt] = prodterm[u] + 10 ** cntforneg
                             #   lenprodterm[newprodscnt] = lenprodterm[u] + 1
                                newprodscnt = newprodscnt + 1
                                otrnowlen = otrnowlen + 1
                                flagout = 1
    ...
    # ### ! ЕСЛИ УБРАТЬ НИЖНЕЕ УСЛОВИЕ, ВРЕМЯ РАБОТЫ АЛГОРИТМА ОЧЕНЬ СИЛЬНО УВЕЛИЧИВАЕТСЯ
                      ...
                      #      print("ok now prodterm is = ", prodterm)
                            ...
                            z = -1
                            gotobeg = 1
                            #if prodterm[0] == 4 and prodterm[3] == 4 and prodterm[1] == 4 and prodterm[4] == 4:
                            #    print("prodterm = ", prodterm)
                            #print("prodterm = ", prodterm)
                            #print("###")
        ...
        #       else: # уже прочитали хотя бы 1 раз
            ...
            #print("here print: numofpols = ", numofpols, " numoflens = ", numoflens)
            cntlens = []
            for cnt in range (0, 2 ** (2 * n)):
                cntlens.append(0)
            minpol = pol
            ...
            if nextloop == 7:  # отвечает за рассматриваемые тождества #17?
                break
